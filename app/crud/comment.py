...
async def create_comment(db: AsyncSession, comment: CommentCreate, user_id: int, post_id: int) -> Comment:
    new_comment = Comment(**comment.model_dump(), user_id=user_id, post_id=post_id)
    db.add(new_comment)
    await db.commit()
